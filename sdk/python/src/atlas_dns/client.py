...
class AtlasDNSClient(BaseClient):
    ...
    def create_record(self, zone_id: str, record: Union[Record, Dict]) -> \
            Record:
        """Create a new DNS record."""
        if isinstance(record, Record):
            record = record.model_dump()
        data = self._request("POST", f"/zones/{zone_id}/records",
                             json_data=record)
        return Record(**data)
    ...
    def bulk_create_records(
        ...
        data = self._request(
            "POST", f"/zones/{zone_id}/records/bulk",
            json_data={"records": records_data}
        )
        return [Record(**record) for record in data]

    def execute_bulk_operation(
        self, operation: Union[BulkOperation, Dict]
    ) -> Dict:
...
class AsyncAtlasDNSClient(BaseClient):
    ...
    # Implementation follows the same pattern as sync client
    ...
    async def update_zone(self, zone_id: str, updates: Dict) -> Zone:
        """Update an existing zone."""
        data = await self._request("PUT", f"/zones/{zone_id}",
                                   json_data=updates)
        return Zone(**data)
